...
export function ItemsManagementStep({
  ...
  const handleScannedItems = (scannedItems: { name: string; price: number }[]) => {
    ...
    scannedItems.forEach((scannedItem) => {
      ...
      // You'll need to update your hook to handle adding multiple items
      // For now, we'll just set the current item
      setCurrentItem({
        name: scannedItem.name,
  ...
  if (showScanner) {
    return (
      <div className="min-h-screen bg-cyan-50 p-4 flex items-center justify-center">
        <ReceiptScanner onItemsScanned={handleScannedItems} onClose={() => setShowScanner(false)} />
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-cyan-50 p-4">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex justify-center mb-4">
          <Button onClick={() => setShowScanner(true)} className="bg-cyan-500 hover:bg-cyan-600 text-white">
