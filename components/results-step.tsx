...
export function ResultsStep({ results, totalAmount, namesCount, onBack, onReset }: ResultsStepProps) {
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <Card className="bg-white border-2 border-glaucous-200 shadow-lg">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-lg text-eerie-600">Here's how much each person owes</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-center mb-8 p-6 bg-vanilla-100 rounded-2xl border-2 border-vanilla-200">
              <div className="text-4xl font-bold text-eerie-900 mb-2">Total: ${totalAmount.toFixed(2)}</div>
              <div className="text-eerie-700">Split among {namesCount} people</div>
            </div>
                ...
                >
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-xl font-bold text-eerie-800">{person.name}</h3>
                      <div className="text-3xl font-bold text-amaranth-600">${person.total.toFixed(2)}</div>
                    </div>
                    <div className="space-y-2">
                      {person.items.map((item, itemIndex) => (
                        <div key={itemIndex} className="flex justify-between text-sm p-2 bg-gray-50 rounded-lg">
                          <span className="text-eerie-700">{item.name}</span>
                          <span className="font-semibold text-glaucous-600">${item.share.toFixed(2)}</span>
                        </div>
                      ))}
                    </div>
                  </CardContent>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-8">
              ...
              <Button onClick={handleShare} className="flex-1 bg-cyan-500 hover:bg-cyan-600 text-white">
                <Share2 className="w-4 h-4 mr-2" />
                Share Results
              </Button>
              <Button onClick={onReset} className="flex-1 bg-amaranth-500 hover:bg-amaranth-600 text-white">
